...
def main():
    ...
    while running:
        ...
        while accumulator >= DT_PHYS:
            ...
            t_s += DT_PHYS
            import time as _tt
            if int(t_s / DT_PHYS) % 5 == 0:
                print("TRACE STEP wall=%.3f t=%.3f" % (_tt.perf_counter(), t_s), flush=True)
            wind.step(DT_PHYS)
        ...
        if render_accum >= 1.0 / SIM_RENDER_HZ:
            import time as _tt
            print("TRACE RENDER wall=%.3f acc=%.4f" % (_tt.perf_counter(), render_accum), flush=True)
            render_accum = 0.0
            frames_rendered += 1
            now2 = time.time()
            if now2 - fps_t > 0.5:
